...
(function(){
  ...
  function toSummary(title){
    show(APP.el.hygienePanel, false);
    APP.state = "SUMMARY";
  ...
  function updateHygieneIndicators(){
    // Update step illustration
    try {
      const id = `#stepImg${APP.hygieneNext}`;
      APP.el.hygieneImage.setAttribute('material', `src: ${'${id}'}`);
      APP.el.hygieneCaption.setAttribute('text','value', `${'${HYGIENE_STEPS[APP.hygieneNext]?.name || "เสร็จสิ้น"}'}`);
    } catch(e) {}
  ...
  function init(){
    ...
    APP.el.hygienePanel = $('hygienePanel');
    APP.el.hygieneImage = $('hygieneImage');
    APP.el.hygieneCaption = $('hygieneCaption');
    APP.hud = $('hud');
